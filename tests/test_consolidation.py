...
需要环境变量 CODEPLUS_TEST_API_KEY、CODEPLUS_TEST_BASE_URL、CODEPLUS_TEST_MODEL。
...
def test_lock_first_acquire():
    from codeplus.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from codeplus.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from codeplus.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from codeplus.memory.consolidation import _build_consolidation_prompt
...
def test_do_consolidation_builds_subagent(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".codeplus", "memory")
...
def test_do_consolidation_subagent_inherits_rules(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".codeplus", "memory")
        setup_test_memories(mem_dir)
        rules_file = Path(work_dir) / ".codeplus" / "permissions.yaml"
...
def test_do_consolidation_subagent_opens_user_memory_dir(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        fake_home = Path(os.path.abspath(os.sep)) / "fake-user-home-for-codeplus-test"
        consolidator._user_mem_dir = str(fake_home / ".codeplus" / "memory")
        ...
        user_mem_file = fake_home / ".codeplus" / "memory" / "MEMORY.md"
...
@pytest.mark.skipif(
    not os.environ.get("CODEPLUS_TEST_API_KEY"),
    reason="CODEPLUS_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["CODEPLUS_TEST_API_KEY"]
    base_url = os.environ.get("CODEPLUS_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("CODEPLUS_TEST_MODEL", "MiniMax-M3")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from codeplus.permissions.checker import PermissionChecker
    from codeplus.tools import ToolRegistry
    from codeplus.tools.bash import Bash
    from codeplus.tools.edit_file import EditFile
    from codeplus.tools.glob import Glob
    from codeplus.tools.grep import Grep
    ...
    from codeplus.client import OpenAICompatClient
    ...
    from codeplus.permissions.rules import RuleEngine
    from codeplus.permissions.dangerous import DangerousCommandDetector
    from codeplus.permissions.checker import PermissionMode
