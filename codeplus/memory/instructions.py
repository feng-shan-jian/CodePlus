...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.codeplus/CODEPLUS.md, ~/.codeplus/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 CODEPLUS.md、AGENTS.md
       和 .codeplus/CODEPLUS.md
    3. workDir/CODEPLUS.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".codeplus" / "CODEPLUS.md")
    _add(home / ".codeplus" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "CODEPLUS.md")
        _add(d / "AGENTS.md")
        # .codeplus/ 下的同名文件：想让指令进 .gitignore 的项目放这里
        _add(d / ".codeplus" / "CODEPLUS.md")

    # 3. 本地覆盖
    _add(root / "CODEPLUS.local.md")
