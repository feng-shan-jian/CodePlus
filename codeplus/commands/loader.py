"""从 .codeplus/commands/ 目录加载自定义 Markdown 命令。
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.codeplus/commands/
      2. <work_dir>/.codeplus/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".codeplus" / "commands"))
    dirs.append(str(Path(work_dir) / ".codeplus" / "commands"))
