...
def _session_path(codeplus_dir: Path) -> Path:
    return codeplus_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(codeplus_dir)
    if session is None:
...
def load_worktree_session(codeplus_dir: Path) -> WorktreeSession | None:
    path = _session_path(codeplus_dir)
    if not path.exists():
        return None
    try:
